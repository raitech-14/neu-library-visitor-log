...
include 'db.php'; // make sure this uses $pdo (NOT $conn)
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // College required for Student & Professor
    ...
    if ($user) {
        ...
        $stmt_update = $pdo->prepare("
            UPDATE users 
            SET full_name = ?, college = ?, visitor_type = ? 
            WHERE email = ?
        ");
        $stmt_update->execute([$full_name, $college, $visitor_type, $email]);

        $user_id = $user['id'];

    } else {

        // ➕ Insert new user (PostgreSQL way)
        $role = "visitor";

        $stmt_insert = $pdo->prepare("
            INSERT INTO users (email, full_name, college, role, visitor_type) 
            VALUES (?, ?, ?, ?, ?)
            RETURNING id
        ");
        $stmt_insert->execute([$email, $full_name, $college, $role, $visitor_type]);

        $user_id = $stmt_insert->fetchColumn();
    }
    ...
    $stmt_log = $pdo->prepare("
        INSERT INTO visit_logs (user_id, purpose, visit_date, visit_time) 
        VALUES (?, ?, ?, ?)
    ");
    $stmt_log->execute([$user_id, $purpose, $visit_date, $visit_time]);
